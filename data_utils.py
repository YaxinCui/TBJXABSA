...
def build_or_load_dataset(args, tokenizer, mode='train',kind=0):
    """
    ...
    if mode == 'dev':
        ...
        elif exp_type == 'acs_kd_s':
            file_name_or_list = [f"gold-{args.src_lang}-train.txt", 
                                 f"cs_{args.src_lang}-{args.tgt_lang}-train.txt",
                                 f"cs_{args.tgt_lang}-{args.src_lang}-train.txt",
                                 f"smt-{args.tgt_lang}-train.txt"]           

        elif exp_type == 'acs_mtl' or exp_type == 'macs_kd':
        ...
        elif 'kd_m' in args.exp_type and kind>=0:
            if kind==0:
                file_name_or_list = [
                                        f"smt-{args.tgt_lang}-train.txt",
                                        f"gold-{args.src_lang}-train.txt"
                                    ]
            elif kind==1:
                file_name_or_list =[f"smt-{args.tgt_lang}-train.txt",
                                    f"cs_{args.tgt_lang}-{args.src_lang}-train.txt"]
            elif kind==2:
                file_name_or_list = [f"smt-{args.tgt_lang}-train.txt",
                                    f"cs_{args.src_lang}-{args.tgt_lang}-train.txt"]
