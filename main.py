...
def init_args():
    ...
    parser.add_argument("--trained_teacher_paths", help="path of the trained model")
    ...
    parser.add_argument('--results_log',help="results_log",type=str, default='results_log')
    parser.add_argument('--outputDIR',help="outputDIR",type=str, default='outputs')
    # distributed training
    parser.add_argument('--student_model_path',help="student_model_path",type=str, default='student_model_path')
...
def train(args, train_dataset, model, tokenizer,kind=-1):
    ...
    test_dataset = build_or_load_dataset(args, tokenizer, mode='test',kind=kind)
...
def get_teacher_model_path(args):
    if 'mtl' in args.exp_type:
        ...
        saved_model_dir = f"{args.outputDIR}/{args.tfm_type}-{args.src_lang}-{args.src_lang}-supervised"
    ...
    print(f"We will perform validation on the following checkpoints: {all_checkpoints},to get get_teacher_model_path")
    ...
    if 'kd_s' in args.exp_type or 'macs_kd' in args.exp_type or 'acs_mtl' in args.exp_type:
        best_checkpoint =''
        best_f1 = -999999.0
        ...
        best_checkpoints = []
        best_f1_g, best_checkpoint_g, best_global_step = -999999.0, None, None
        for kind in range(3):
            best_checkpoint =''
            best_f1 = -999999.0
            for checkpoint in all_checkpoints:
                ...
                if True:
                    ...
                    if dev_result[metrics] > best_f1:
                        best_f1 = dev_result[metrics]
                        best_checkpoint = checkpoint
                    if dev_result[metrics] > best_f1_g:
                        best_f1_g = dev_result[metrics]
...
def main():
    ...
    if args.do_distill:
        ...
        if args.exp_type == 'acs_kd_s':
            ...
            args.student_model_path=teacher_model_path
            args.trained_teacher_paths=teacher_model_path
        ...
        # use the three combinations as multi-teacher distill
            ...
            teacher_model_path = get_teacher_model_path(args)
            args.student_model_path=teacher_model_path[-1]
            args.trained_teacher_paths=teacher_model_path[:3]
            teacher_probs = get_multi_teacher_probs(args, dataset, model_class)
        # MTL version of distillation
        elif args.exp_type == 'macs_kd' or args.exp_type == 'acs_mtl':
            teacher_model_path = get_teacher_model_path(args)
            args.student_model_path=teacher_model_path
            args.trained_teacher_paths=teacher_model_path
            teacher_probs = get_teacher_probs(args, dataset, model_class, teacher_model_path)
    ...
    best_checkpoint=f'{args.outputDIR}/bert-ner-en-en-supervised-2.5-all-bert-base-cased/checkpoint-100'
    if args.do_eval:
        ...
        target_train_dataset = build_or_load_dataset(args, tokenizer, mode='train')
        ...
        source_test_dataset = build_or_load_dataset(args, tokenizer, mode='test')
        for checkpoint in all_checkpoints:
            global_step = checkpoint.split('-')[-1] if len(checkpoint) > 1 else ""
...
if __name__ == '__main__':
    #os.chdir("/content/drive/MyDrive/XABSA/XABSA")
    main()